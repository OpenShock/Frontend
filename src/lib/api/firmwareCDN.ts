...
const boardFileUrl = (version: string, board: string, fileName: string) => `${BASE_URL}/${version}/${board}/${fileName}`;
...
export async function GetBoardBinaryHashes(version: string, board: string, hashType: 'md5' | 'sha256' = 'sha256') {
  ...
  for (const line of lines) {
    ...
    const filename = parts.slice(1).join(' ').trim(); // Join the rest in case filename has spaces
...
export async function GetBoardBinaryHash(version: string, board: string, filename: string, hashType: 'md5' | 'sha256' = 'sha256') {
...
export async function DownloadAndVerifyBoardBinary(
  ...
  const [binary, hash] = await Promise.all([
    ...
    GetBoardBinaryHash(version, board, filename)
  ]);
  ...
  if (calculatedHash !== hash) {
    throw new Error(`Hash mismatch for ${filename} in board ${board} version ${version}: expected ${hash}, got ${calculatedHash}`);
  }
